...
const Header = () => {
  ...
  const timmer=  setTimeout(() => { // 
    searchSuggestions();
  }, 200);  // Suppose when we clicked on the key i -> its change the state so, the use state will be call use Effect which have function which render after 200ms --> but if i have place another key now it will again do the same process but it will take next 200 ms  so, basically it call  API on evry chnage of state 
